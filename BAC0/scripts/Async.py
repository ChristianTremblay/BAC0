...
from BAC0.core.app.asyncApp import run, loop
...
class Async(
    ...
    async def discover(
        ...
        _this_network = await self.this_application.app.nse.what_is_network_number()
        _networks.append(_this_network)
        # Try to find local routers...
        _other_networks = await self.this_application.app.nse.who_is_router_to_network()
        for each in _other_networks:
            network_adapter, _iamrtn = each
            _networks.extend(_iamrtn.iartnNetworkList)
        self.this_application._learnedNetworks = _networks
        ...
        if _networks:
            for network in _networks:
                ...
                for each in _res:
                    found.append(each.iAmDeviceIdentifier)
            ...
            _res = await self.this_application.app.self.who_is(
                low_limit=deviceInstanceRangeLowLimit,
                high_limit=deviceInstanceRangeHighLimit,
